<template>
  ...
  <v-row>
    <v-col v-if="state.show === 0" cols="12" md="6" class="mx-auto">
      <div class="mb-3">
    ...
    <v-col v-else md="4" cols="12" class="mx-auto">
      <div class="d-flex mb-3">
        <p class="h56">Point Of Sale</p>
        <v-spacer />
        <v-btn flat density="compact" prepend-icon="mdi-arrow-left" class="px-0" @click="() => (state.show = 0)">
          Back
        </v-btn>
      </div>
...
export default {
  setup() {
    const state = reactive({ show: 1, amount: "0" })
